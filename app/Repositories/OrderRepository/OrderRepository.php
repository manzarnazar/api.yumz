...
public function ordersReportInvoice(array $filter): array
{
    ...
    // Format breakdown by date
    $breakdown = $statistic->map(function($item) {
        return [
            'date' => \Carbon\Carbon::parse($item->order_date)->format('d/m'),
            'orders' => $item->total_orders,
            'revenue' => $item->total_prices,
            'commission_fee' => $item->total_commission_fee,
            'total_payable' => $item->total_prices - $item->total_commission_fee,
        ];
    });

    // Calculate totals for the summary
    $totalRevenue = $statistic->sum('total_prices');
    $totalOrders = $statistic->sum('total_orders');
    $totalCommission = $statistic->sum('total_commission_fee');
    $totalPayable = $totalRevenue - $totalCommission;
